 ...
 * @returns 返回能出的最小手牌
 */
function selectCardsRobot(HoldCard, CurrentCards) { //手持牌  牌堆牌  准备出的牌
    let ReadyCard = [];
                ...
                if (c1.length != 0) {
                    for (let i = 0; i < c3.length; i += 3) {
                        if (parseInt(c3[i] / 4) > cur) {
                            ...
                            /* for (let j = 0; j < HoldCard.length; j++) {
                                if (HoldCard[j] == c3[i + 1]) {
                                    HoldCard.splice(j, 1);
                                }
                            } */
                            ReadyCard.push(c3[i + 2]);
                            /* for (let j = 0; j < HoldCard.length; j++) {
                                if (HoldCard[j] == c3[i + 2]) {
                                    HoldCard.splice(j, 1);
                                }
                            } */
                            ReadyCard.push(c1[0]);
    ...
    if (ReadyCard.length == 0) {
        if (HoldCard.indexOf(52) != -1 && HoldCard.indexOf(53) != -1) {
            ...
            ReadyCard.push(53);
            /* for (let j = 0; j < HoldCard.length; j++) {
                if (HoldCard[j] == 52 || HoldCard[j] == 53) {
                    HoldCard.splice(j, 1);
                }
            } */
        }
